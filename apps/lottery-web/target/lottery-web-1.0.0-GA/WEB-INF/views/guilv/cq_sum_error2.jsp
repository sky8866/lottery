...
<body>
	...
	<div class="box-body chat" id="chat-box">
        ...
        <div class="col-md-12">
        <c:choose>
			<c:when test="${!empty removeCounts || !empty peerList ||!empty peer3List||!empty sbr||!empty scopeList||!empty singleDoubleList}">
        
        
               <c:forEach items="${removeCounts }" var="entry" varStatus="statu"> 
      <div class="item">
						<c:choose>
						...
						<p class="message">
							<a href="###" onclick="importDetail('${entry.id }','${entry.gui }')"    class="name">
								   <small class="text-muted pull-right"><i class="glyphicon glyphicon-dashboard"></i> ${entry.qi }期</small>  
								 排除:${entry.value }${entry.valueType } </a> ${entry.queryValue }<br> <span
