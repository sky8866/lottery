...
@Controller 
public class GuilvPushAjaxController {
	...
	@RequestMapping(value = "/admin/ajax/guilv/push")
	public String query(
			String openId,
		...
		if(user!=null&&user.getIsVip()==1){
			PageView<GuilvSum> pageView = new PageView<GuilvSum>(5,1);
			pageView.setOrderby("id", "desc");
			List<GuilvSum> guilvSums=this.guilvSumService.findList(pageView, null).getResultlist();
