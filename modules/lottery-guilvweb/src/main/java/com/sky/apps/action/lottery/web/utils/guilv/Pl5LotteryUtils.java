...
public class Pl5LotteryUtils {
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,Pl5LotteryService lotteryService,Pl5GuilvService guilvService,List<Pl5Guilv> sbr,boolean isvip,Pl5GuilvRemoveCountService countService,Pl5GuilvRemoveConditionService conditionService){
		...
		if (guilvs.isEmpty()) {
			...
			Pl5GuilvRemoveCondition	conditions=conditionService.findGuilv(fixation, buchang, addition, number, 1);
	...
	private static void guilv(int type, int sum, int query, List<Pl5Lottery> lotteries,
			List<StringBuffer> sbs, List<Pl5Guilv> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,long qi, Pl5GuilvService guilvService,Pl5GuilvRemoveCondition	conditions) {
	...
	private static void getFixation(int type, int sum, int query,
			List<Pl5Lottery> lotteries, List<StringBuffer> sbs, List<Pl5Guilv> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,long qi, Pl5GuilvService guilvService,Pl5GuilvRemoveCondition	conditions) {
		int z = s(sum);
		int x = z + e;
	...
	private static void one(int x, int type, int query, List<Pl5Lottery> lotteries,
			List<StringBuffer> sbs, List<Pl5Guilv> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,long qi, Pl5GuilvService guilvService,Pl5GuilvRemoveCondition	conditions) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
                    ...
                    vo.setGui(1);
                       ...
                       Pl5NumCountUtils.getGuilv(addition, buchang, fixation, number, vo, vo.getSum(), infoString);
    
					 if(StringUtils.isNotEmpty(vo.getInfo())){
						 
						 vo.setNum(getOldLottery(vo.getSum(),addition, buchang, even, e, qi, matchType, guilvService));
	...
	public static void guilvRemove(Pl5Guilv g,Pl5GuilvRemoveCondition conditions,Pl5GuilvRemoveCountService cocuntService,Pl5GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(Pl5Guilv g,Pl5GuilvRemoveCountService countService,int remove,Pl5GuilvRemoveCondition	conditions){
	...
	public static void condition(Pl5GuilvRemoveCondition condition,Pl5Guilv g,Pl5GuilvRemoveConditionService conditionService ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType,Pl5GuilvService guilvOldService){
		PageView<Pl5Guilv> pageViewGuilv = new PageView<Pl5Guilv>();
		List<PropertyFilter> filtersnewses = new ArrayList<PropertyFilter>();
		filtersnewses
				.add(new PropertyFilter("addition", MatchType.EQ, addition));
		...
		List<Pl5Guilv> gs=guilvOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
