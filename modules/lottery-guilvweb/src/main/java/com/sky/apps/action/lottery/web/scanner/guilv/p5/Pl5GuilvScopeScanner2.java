...
@Service("Pl5GuilvScopeScanner2") 
public class Pl5GuilvScopeScanner2 implements QueryComm{

	@Autowired
	private Pl5LotteryService lotteryService;
	@Autowired Pl5GuilvRemoveCountService guilvRemoveCountService; 
	@Autowired
	private Pl5GuilvScopeOldService guilvOLdService; 
	@Autowired Pl5GuilvRemoveConditionService  conditionsService;
	public void job() {
	 
		 System.out.println("p5_scope2");
					...
					if(fixation==1){
						...
						for(int type=0;type<=3;type++){
							 for(int number=6;number<=6;number++){
						        ...
						        Pl5LotteryGuilvScopeUtils.setQuery(addition, b, type,NumCountUtils.getCount(addition, b, f, number), fixation, 1, number, lotteryService, guilvOLdService,  true,guilvRemoveCountService,conditionsService);
