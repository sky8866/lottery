...
public class Pl5LotteryGuilvScopeUtils {
	private	static Pl5GuilvRemoveCountService guilvRemoveCountService;
	private	static Pl5GuilvScopeOldService oScopeOLdService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,Pl5LotteryService lotteryService,Pl5GuilvScopeOldService guilvService,boolean isvip,Pl5GuilvRemoveCountService countService,Pl5GuilvRemoveConditionService conditionService){
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<Pl5GuilvScopeOld> guilvs = pageViewGuilv.getRecords();
		
		if (guilvs.isEmpty()) {
			 Pl5GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 2);
			...
			if (bsum) {
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					Pl5GuilvScopeOld vo = new Pl5GuilvScopeOld();
                    ...
                    vo.setGui(1);
                       ...
                       Pl5NumCountUtils.getGuilvScope(addition, buchang, fixation, number, vo, vo.getSum(), infoString);
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType){
		...
		List<Pl5GuilvScopeOld> gs=oScopeOLdService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(Pl5GuilvScopeOld g:gs){
	...
	public static void guilvRemove(Pl5GuilvScopeOld g, Pl5GuilvRemoveCondition conditions,Pl5GuilvRemoveCountService cocuntService,Pl5GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(Pl5GuilvScopeOld g,Pl5GuilvRemoveCountService countService,int remove,Pl5GuilvRemoveCondition	conditions){
	...
	public static void condition(Pl5GuilvRemoveCondition condition,Pl5GuilvScopeOld g,Pl5GuilvRemoveConditionService conditionService ){
