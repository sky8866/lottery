...
@Service("CqGuilvScanner213") 
public class CqGuilvScanner213 implements QueryComm{
	...
	public void job() {
						...
						for(int type=0;type<=4;type++){ 
							 for(int number=6;number<=6;number++){
								 ...
								 for(int addition=3;addition<=3;addition++){
								    ...
								    CqLotteryUtils.setQuery(addition, buchang, type, CqNumCountUtils.getCount(addition, buchang, 1, number), 1, 1, number, lotteryService, guilvService, sbr,true,cocuntService,conditionService);	
		...
		System.out.println("cq_guilv2结束3：："+UtilDate.getDateFormatter()); 
