...
public class LotteryPeerUtils {
	private	 static GuilvPeerOldService peerOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,LotteryService lotteryService,GuilvPeerService guilvService,List<GuilvPeer> sbr,boolean isvip,GuilvPeerOldService guilvOldService,  GuilvRemoveCountService countService,GuilvRemoveConditionService conditionService){
		...
		PageView<GuilvPeer> pageViewGuilv = new PageView<GuilvPeer>();
		...
		if (guilvs.isEmpty()) {
			GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 4);
	...
	private static void guilv(int type, int sum, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<GuilvPeer> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,int qi, GuilvPeerService guilvService,List<StringBuffer> sbs2,GuilvRemoveCondition  conditions) {
	...
	private static void getFixation(int type, int sum, int query,
			List<Lottery> lotteries, List<StringBuffer> sbs, List<GuilvPeer> sbr,
			List<String> sumVaule, int even, int fixation, int matchType, int number,int addition,int buchang,int qi, GuilvPeerService guilvService,List<StringBuffer> sbs2,GuilvRemoveCondition  conditions) {
		int z = s(sum);
		int x = z + fixation;
	...
	public static void guilvRemove(GuilvPeerOld g,GuilvRemoveCondition conditions,GuilvRemoveCountService cocuntService,GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(GuilvPeerOld g,GuilvRemoveCountService countService,int remove,GuilvRemoveCondition	conditions){
	...
	private static void condition(GuilvRemoveCondition condition,GuilvPeerOld g ){
	...
	private static int getOldLottery(int sum, int addition, int buchang, int even,int fixation,int qi, int matchType ){
		...
		List<GuilvPeerOld> gs=peerOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(GuilvPeerOld g:gs){
