...
@Service("GuilvSingleDoubleScanner2")
public class GuilvSingleDoubleScanner2  implements QueryComm   {
	...
	@Autowired GuilvRemoveCountService guilvRemoveCountService;
		...
		@Autowired
		private GuilvSingleDoubleOLdService goService;
		...
		System.out.println("singleDouble2");
	...
	private void s2(int addition,int buchang,int type,int fixation, LotteryService lotteryService, GuilvSingleDoubleService guilvService){
	    ...
	    LotteryGuilvSingleDoubleUtils.setQuery(addition, buchang, type, count, fixation, 1, 5, lotteryService, guilvService, sbr,true,goService,guilvRemoveCountService,conditionsService);	
