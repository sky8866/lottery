...
import com.sky.apps.action.lottery.web.utils.guilv.XjsscGuilvKillUtils;
import com.sky.apps.action.lottery.web.utils.guilv.XjsscGuilvUtils;
import com.sky.apps.action.lottery.web.vo.guilv.ComparatorXjsscGuilv;
...
@Service("XjsscGuilvScanner") 
public class XjsscGuilvScanner implements QueryComm{

	@Autowired
	private XjsscLotteryService lotteryService;
	
	@Autowired
	private XjsscGuilvService guilvService;
	
	@Autowired
	private XjsscGuilvKillService guilv4Service;
	...
	private void setPlan(long qi){
		...
		XjsscPlan plan2=planService.findPlan(qi+1,1);
		
		 
		if(plan2==null){
		
		XjsscPlan plan=planService.findPlan(1);
		
		 XjsscLottery lottery2=lotteryService.findReport("qi", qi);
			...
			for(int type=0;type<=4;type++){
				...
				ComparatorXjsscGuilv guilv = new ComparatorXjsscGuilv();
	...
	private void setPlan8(long qi,List<XjsscLottery> lotteries){
		...
		XjsscPlan plan7=planService.findPlan(7);
	 
		 
	  
		XjsscPlan plan8=planService.findPlan(8);
	 
		 
		 
		 
		XjsscPlan plan9=planService.findPlan(9);
		 
		
	 
	 
		XjsscPlan plan10=planService.findPlan(10);
		 
		
		 
			 
		XjsscPlan plan11=planService.findPlan(11);
	 
		 
		 
		XjsscPlan plan12=planService.findPlan(15);
		...
		StringBuffer gg1=new StringBuffer();;StringBuffer gg2=new StringBuffer();;StringBuffer gg3=new StringBuffer();;
			...
			*/
			...
			XjsscPlan	plan2=planService.findPlan(qi+1, 6);
			if(plan2==null){
	  ...
	  XjsscPlan	plan77=planService.findPlan(qi+1, 7);
	  if(plan77==null){
			  
			 plan77= new XjsscPlan();
			    if(plan7!=null){
			...
			try{
	  XjsscPlan  plan88=planService.findPlan(qi+1, 8);
	  if(plan88==null){
		  
	  
			  
			  
			  plan88=new XjsscPlan();
			  if(plan8!=null){
			     int count=0;
   				String ss4="";
   				if(f1>=2){
			...
			try{
			  ...
			  XjsscPlan	plan99=planService.findPlan(qi+1, 9);
			  if(plan99==null){
					  plan99=new XjsscPlan();
			...
			try{
				  ...
				  XjsscPlan	plan100=planService.findPlan(qi+1, 10);
				  if(plan100==null){
						  
					  
				  
					  plan100= new XjsscPlan();
			...
			try{
					  ...
					  XjsscPlan	plan110=planService.findPlan(qi+1, 11);
					  if(plan110==null){
							  plan110=new XjsscPlan();
			...
			try{
						  XjsscPlan	plan120=planService.findPlan(qi+1, 15);
						  if(plan120==null){
							  plan120= new XjsscPlan();
	...
	private void setPlan3(long qi,List<XjsscLottery> lotteries){
	    
			XjsscPlan plan5=planService.findPlan(qi+1,5);
			 
			 
			  if(plan5==null){  
			
			XjsscPlan plan=planService.findPlan(5);
			...
			XjsscPlan	plan2=new XjsscPlan();
				...
				for(int type=0;type<=4;type++){
					...
					if(type==0){
						...
						XjsscGuilv k=guilvService.findGuilvKill3(qi+1, type) ;
						if(k!=null){
							 plan2.setNumber1(k.getValue()) ;
						 }else{
							 
							 XjsscGuilvKill  kill2=guilv4Service.findGuilvKill4(qi+1, type);
								...
								if(kill2!=null){
									plan2.setNumber1(kill2.getValue());
									 ...
									 XjsscGuilvKill  kill3=guilv4Service.findGuilvKill(qi+1, type,0,6,2);
									
									if(kill3!=null){
										plan2.setNumber1(kill3.getValue().substring(0,1));
									}else{
										 XjsscGuilvKill  kill4=guilv4Service.findGuilvKill(qi+1, type,2,6,2);
										 if(kill4!=null){
											 plan2.setNumber1(kill4.getValue().substring(0,1));
										 }else{
											 XjsscGuilvKill  kill5=guilv4Service.findGuilvKill(qi+1, type,3,7,3); 
											 if(kill5!=null){
												 plan2.setNumber1(kill5.getValue().substring(0,1)); 
											 }else{
												 XjsscGuilvKill  kill6=guilv4Service.findGuilvKill(qi+1, type,3,6,2);
												 if(kill6!=null){
													 plan2.setNumber1(kill6.getValue().substring(0,1));  
												 }else{
													 XjsscGuilvKill  kill7=guilv4Service.findGuilvKill(qi+1, type,2,5,2); 
													 if(kill7!=null){
														 plan2.setNumber1(kill7.getValue().substring(0,1)); 
													 }else{
														 XjsscGuilvKill  kill8=guilv4Service.findGuilvKill(qi+1, type,0,5,2);  
														 if(kill8!=null){
	...
	private void setPlan5(long qi,List<XjsscLottery> lotteries){
	    
		XjsscPlan plan5=planService.findPlan( qi+1,12);
		 
		 
		  if(plan5==null){  
		
		XjsscPlan plan=planService.findPlan( qi,12);
		 XjsscLottery lottery2=lotteryService.findReport("qi", qi);
		int co=0;
		int num=0;
	...
	private void setPlan6(long qi,List<XjsscLottery> lotteries){
	    
		XjsscPlan plan5=planService.findPlan( qi+1,13);
		 
		 
		  if(plan5==null){  
		
		XjsscPlan plan=planService.findPlan( qi,13);
		 XjsscLottery lottery2=lotteryService.findReport("qi", qi);
		int co=0;
		int num=0;
	...
	private static void getValue(StringBuffer sBuffer,long qi,int type,XjsscGuilvKillService guilv4Service,
			XjsscGuilvService guilvService,List<XjsscLottery> lotteries,StringBuffer cc1,StringBuffer cc2,StringBuffer cc3
			){
		...
		if(sBuffer.length()<3){
			...
			List<XjsscGuilv> kills=guilvService.findLists(qi, num,type);
			for(XjsscGuilv g:kills){
				 String value="";
		...
		if(sBuffer.length()<3){
			List<XjsscGuilvKill> kill2 = guilv4Service.findLists(qi+1,
					10, type, 2, 5);

			for (XjsscGuilvKill g : kill2) {
		...
		if(sBuffer.length()<3){
		 ...
		 if(sBuffer.length()<3){
				List<XjsscGuilvKill> kill3 = guilv4Service.findLists(qi+1,
						7, type, 1, 5);

				for (XjsscGuilvKill g : kill3) {
		...
		if(sBuffer.length()<3){
			List<XjsscGuilv> kills=guilvService.findLists3(qi+1, type,1);
			for(XjsscGuilv g:kills){
				if(sBuffer.indexOf(g.getValue())==-1){
					sBuffer.append(g.getValue());
					c2++;
		...
		if(sBuffer.length()<3){
			List<XjsscGuilv> kills=guilvService.findLists3(qi+1, type,2);
			for(XjsscGuilv g:kills){
				if(sBuffer.indexOf(g.getValue())==-1){
					sBuffer.append(g.getValue());
					c2++;
		...
		if(sBuffer.length()<3){
			...
			List<XjsscGuilv> kills=guilvService.findLists(qi-1, num,type);
			for(XjsscGuilv g:kills){
				 String value="";
				 String string=g.getValueNew().replaceAll("\\[", "").replaceAll("\\]", "");
				 String[] s=string.split(",");
		...
		if(sBuffer.length()<3){
			...
			List<XjsscGuilv> kills=guilvService.findLists(qi-2, num,type);
			for(XjsscGuilv g:kills){
				 String value="";
				 String string=g.getValueNew().replaceAll("\\[", "").replaceAll("\\]", "");
				 String[] s=string.split(",");
		...
		if(sBuffer.length()<3){
			List<XjsscGuilv> kills=guilvService.findLists5(type, 5);
			for(XjsscGuilv g:kills){
				 String value="";
				 String string=g.getValueNew().replaceAll("\\[", "").replaceAll("\\]", "");
				 String[] s=string.split(",");
 ...
 cc3.append(c3+"");
	...
	private void s2(int addition,int b,int type,int fixation, XjsscLotteryService lotteryService, XjsscGuilvService guilvService,List<XjsscLottery> lotteries){
		  
	       
	    List<XjsscGuilv> sbr = new ArrayList<XjsscGuilv>();
 
            int count=1;
			  
	    XjsscGuilvUtils.setQuery(addition, b, type, count, fixation, 1, 1, lotteryService, guilvService, sbr,true,lotteries);	
		 
	}
 
	
	private void s3(int addition,int b,int type,int fixation, XjsscLotteryService lotteryService, XjsscGuilvKillService guilv4Service,List<XjsscLottery> lotteries){
		  
	       for(int n=0;n<4;n++){
	    	   
	    	   List<XjsscGuilvKill> sbr = new ArrayList<XjsscGuilvKill>();
	    	   int count=4;
	           if(n==0&&addition==2){
	        	   count=4;
		   ...
		   XjsscGuilvKillUtils.setQuery(addition, b, type, count, fixation, 1, n, lotteryService, guilv4Service, sbr,true,lotteries);   
	       }
