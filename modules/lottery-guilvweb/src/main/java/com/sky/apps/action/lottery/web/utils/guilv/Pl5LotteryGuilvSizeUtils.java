...
public class Pl5LotteryGuilvSizeUtils {
	private static Pl5GuilvSizeOldService guilvOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,Pl5LotteryService lotteryService,Pl5GuilvSizeOldService guilvService,List<Pl5GuilvSizeOld> sbr,boolean isvip,Pl5GuilvRemoveCountService countService,Pl5GuilvRemoveConditionService conditionService){
		...
		PageView<Pl5GuilvSizeOld> pageViewGuilv = new PageView<Pl5GuilvSizeOld>();
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<Pl5GuilvSizeOld> guilvs = pageViewGuilv.getRecords();
		
		if (guilvs.isEmpty()) {
			Pl5GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 6);
	...
	public static String getVaule(List<Pl5GuilvSizeOld> sbr, int type) {
		String stypeString = "";
		if (type == 0) {
			stypeString = "头";
		} else if (type == 1) {
	...
	private static void guilv(int type, int sum, int query, List<Pl5Lottery> lotteries,
			List<StringBuffer> sbs, List<Pl5GuilvSizeOld> sbr, List<String> sumVaule,
	...
	private static void getFixation(int type, int sum, int query,
			List<Pl5Lottery> lotteries, List<StringBuffer> sbs, List<Pl5GuilvSizeOld> sbr,
			...
			if (bsum) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					Pl5GuilvSizeOld vo = new Pl5GuilvSizeOld();
	...
	public static void guilvRemove(Pl5GuilvSizeOld g,Pl5GuilvRemoveCondition conditions,Pl5GuilvRemoveCountService cocuntService,Pl5GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(Pl5GuilvSizeOld g,Pl5GuilvRemoveCountService countService,int remove,Pl5GuilvRemoveCondition	conditions){
	...
	public static void condition(Pl5GuilvRemoveCondition condition,Pl5GuilvSizeOld g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType){
		...
		List<Pl5GuilvSizeOld> gs=guilvOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(Pl5GuilvSizeOld g:gs){
