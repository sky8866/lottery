...
@Service("CqGuilvScanner211") 
public class CqGuilvScanner211 implements QueryComm{
						...
						for(int type=0;type<=4;type++){ 
							 for(int number=6;number<=6;number++){
								 ...
								 for(int addition=1;addition<=1;addition++){
									 ...
									 for(int fixation=1;fixation<2;fixation++){
											int f=0;
											if(fixation==1){
												f=10;
											}
								    ...
								    CqLotteryUtils.setQuery(addition, b, type, CqNumCountUtils.getCount(addition, b, f, number), f, 1, number, lotteryService, guilvService, sbr,true,cocuntService,conditionService);	
	...
	private void s2(int addition,int buchang,int type,int fixation, CqsscLotteryService lotteryService, CqGuilvService guilvService){
	    ...
	    CqLotteryUtils.setQuery(addition, buchang, type, CqNumCountUtils.getCount(addition, buchang, fixation, number), fixation, 1, number, lotteryService, guilvService, sbr,true,cocuntService,conditionService);	
