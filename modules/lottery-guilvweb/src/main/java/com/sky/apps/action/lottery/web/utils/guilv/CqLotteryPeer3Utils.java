...
public class CqLotteryPeer3Utils {
	private	 static CqGuilvPeer3OldService peerOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,CqsscLotteryService lotteryService,CqGuilvPeer3OldService guilvService,List<CqGuilvPeer3Old> sbr,boolean isvip,CqGuilvRemoveCountService countService,CqGuilvRemoveConditionService conditionService){
		 even=even-2;
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<CqGuilvPeer3Old> guilvs = pageViewGuilv.getRecords();
		
		int query = sum(lotteries, 3, type, sbs2.get(0)); 
	 
		if (guilvs.isEmpty()) {
			 CqGuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 5);
	...
	private static void guilv(int type, int sum, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqGuilvPeer3Old> sbr, List<String> sumVaule,
	...
	private static void getFixation(int type, int sum, int query,
			List<CqsscLottery> lotteries, List<StringBuffer> sbs, List<CqGuilvPeer3Old> sbr,
		 	  ...
		 	  int countold = setold( fixation,type,sbs, lotteries,  number,count,sbs2); 
				...
				if (flag) {
					...
					qu=qu.substring(0,qu.length()-1);
						...
						CqGuilvPeer3Old vo = new CqGuilvPeer3Old();
	...
	public static void guilvRemove(CqGuilvPeer3Old g,CqGuilvRemoveCondition	conditions,CqGuilvRemoveCountService cocuntService,CqGuilvRemoveConditionService conditionService){
	...
	private static void removeCount(CqGuilvPeer3Old g,CqGuilvRemoveCountService countService,int remove,CqGuilvRemoveCondition	conditions){
	...
	public static void condition(CqGuilvRemoveConditionService conditionService,CqGuilvRemoveCondition condition,CqGuilvPeer3Old g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType ){
		...
		List<CqGuilvPeer3Old> gs=peerOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(CqGuilvPeer3Old g:gs){
	...
	public  static String getQueryValue(List<CqsscLottery> lotteries, int j, int type) {
		String lString = "";
		try{
