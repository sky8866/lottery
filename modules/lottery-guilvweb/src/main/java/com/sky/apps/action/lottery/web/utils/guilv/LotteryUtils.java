...
public class LotteryUtils {
	private	static GuilvOLdService guilvOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,LotteryService lotteryService,GuilvService guilvService,List<Guilv> sbr,boolean isvip,GuilvOLdService oldService,GuilvRemoveCountService countService,GuilvRemoveConditionService conditionService){
		...
		PageView<Guilv> pageViewGuilv = new PageView<Guilv>();
		...
		if (guilvs.isEmpty()) {
			  ...
			  GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 1);
	...
	public static String getVaule(List<Guilv> sbr, int type) {
		String stypeString = "";
		if (type == 0) {
			stypeString = "头";
		} else if (type == 1) {
	...
	private static void guilv(int type, int sum, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<Guilv> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,int qi, GuilvService guilvService,  GuilvRemoveCondition  conditions) {
	...
	private static void getFixation(int type, int sum, int query,
			List<Lottery> lotteries, List<StringBuffer> sbs, List<Guilv> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,int qi, GuilvService guilvService,  GuilvRemoveCondition  conditions) {
		int z = s(sum);
		int x = z + e;
	...
	private static void one(int x, int type, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<Guilv> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,int qi, GuilvService guilvService,  GuilvRemoveCondition  conditions) {
		 
	/*	 sbs=getSb(5);
		 sum(lotteries, 2, 0, sbs.get(0));
		 sum(lotteries, 3, 2, sbs.get(1));
		 sum(lotteries, 6, 1, sbs.get(2));
		 sum(lotteries, 8, 4, sbs.get(3));
		 sum(lotteries, 8, 6, sbs.get(4));*/
		if (more(s(x), query, number)) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
                    ...
                    vo.setGui(1);
                       ...
                       NumCountUtils.getGuilv(addition, buchang, fixation, number, vo, vo.getSum(), infoString);
         ...
         /*              if(number==1&&buchang<=5&&addition<=5&&vo.getSum()>=2+addition){
						...
						vo.setInfo(infoString);
					} else if(number==6&&buchang>=5&&addition==2&&vo.getSum()>15){
						vo.setInfo(infoString);
					} else if(number==5&&buchang>=5&&addition==2&&vo.getSum()>13){
						vo.setInfo(infoString);
					}  else if(number==4&&buchang>=5&&addition==2&&vo.getSum()>12){
						vo.setInfo(infoString);
					}  else if(number==4&&buchang>=5&&addition==2&&vo.getSum()>11){
						vo.setInfo(infoString);
					}*/   
					 guilvService.saveGuilv(vo); 
					 if(StringUtils.isNotEmpty(vo.getInfo())){
						 GuilvOld old=new GuilvOld(vo);
	...
	public static void guilvRemove(GuilvOld g, GuilvRemoveCondition conditions,GuilvRemoveCountService cocuntService,GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(GuilvOld g,GuilvRemoveCountService countService,int remove,GuilvRemoveCondition	conditions){
		Object[] objects=new Object[5];
		 ...
		 if(count==null){
			 ...
			 count.setInfo(g.getInfo());
	    	    ...
	    	    count.setQi(g.getQi());
	    	    count.setQueryValue("【综合规】"+g.getQueryValue());
	...
	private static void condition(GuilvRemoveCondition condition,GuilvOld g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,int qi, int matchType,GuilvOLdService guilvOldService){
		...
		List<GuilvOld> gs=guilvOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
