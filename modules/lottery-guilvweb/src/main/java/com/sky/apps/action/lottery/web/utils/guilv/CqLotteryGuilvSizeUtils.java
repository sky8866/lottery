...
public class CqLotteryGuilvSizeUtils {
	private	static CqGuilvSizeOldService guilvOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,CqsscLotteryService lotteryService,CqGuilvSizeOldService guilvService,List<CqGuilvSizeOld> sbr,boolean isvip,CqGuilvRemoveCountService countService,CqGuilvRemoveConditionService conditionService){
		 even=even-2;
		...
		PageView<CqGuilvSizeOld> pageViewGuilv = new PageView<CqGuilvSizeOld>();
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<CqGuilvSizeOld> guilvs = pageViewGuilv.getRecords();
		
		if (guilvs.isEmpty()) {
			  CqGuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 6);
	...
	/*	ComparatorGuilvSize guilv = new ComparatorGuilvSize();
	...
	public static String getVaule(List<CqGuilvSizeOld> sbr, int type) {
		String stypeString = "";
		if (type == 0) {
			stypeString = "万";
		} else if (type == 1) {
	...
	private static void guilv(int type, int sum, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqGuilvSizeOld> sbr, List<String> sumVaule,
	...
	private static void getFixation(int type, int sum, int query,
			List<CqsscLottery> lotteries, List<StringBuffer> sbs, List<CqGuilvSizeOld> sbr,
			...
			if (bsum) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					CqGuilvSizeOld vo = new CqGuilvSizeOld();
...
public static void guilvRemove(CqGuilvSizeOld g,CqGuilvRemoveCondition	conditions,CqGuilvRemoveCountService cocuntService,CqGuilvRemoveConditionService conditionService){
	...
	private static void removeCount(CqGuilvSizeOld g,CqGuilvRemoveCountService countService,int remove,CqGuilvRemoveCondition	conditions){
	...
	public static void condition(CqGuilvRemoveConditionService conditionService,CqGuilvRemoveCondition condition,CqGuilvSizeOld g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType){
		...
		List<CqGuilvSizeOld> gs=guilvOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(CqGuilvSizeOld g:gs){
