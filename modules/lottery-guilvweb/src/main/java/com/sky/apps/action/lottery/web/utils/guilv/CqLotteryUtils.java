...
public class CqLotteryUtils {
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,CqsscLotteryService lotteryService,CqGuilvService guilvService,List<CqGuilv> sbr,boolean isvip,CqGuilvRemoveCountService countService,CqGuilvRemoveConditionService conditionService){
	  even=even-2;
		...
		if (guilvs.isEmpty()) {
			...
			CqGuilvRemoveCondition	conditions=conditionService.findGuilv(fixation, buchang, addition, number, 1);
	...
	private static void guilv(int type, int sum, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqGuilv> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,long qi, CqGuilvService guilvService,CqGuilvRemoveCondition	conditions) {
	...
	private static void getFixation(int type, int sum, int query,
			List<CqsscLottery> lotteries, List<StringBuffer> sbs, List<CqGuilv> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,long qi, CqGuilvService guilvService,CqGuilvRemoveCondition	conditions) {
		int z = s(sum);
		int x = z + e;
	...
	private static void one(int x, int type, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqGuilv> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,long qi, CqGuilvService guilvService,CqGuilvRemoveCondition	conditions) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
                    ...
                    vo.setGui(1);
                       ...
                       CqNumCountUtils.getGuilv(addition, buchang, fixation, number, vo, vo.getSum(), infoString);
	...
	public static void guilvRemove(CqGuilv g,CqGuilvRemoveCondition	conditions,CqGuilvRemoveCountService cocuntService,CqGuilvRemoveConditionService conditionService){
	...
	private static void removeCount(CqGuilv g,CqGuilvRemoveCountService countService,int remove,CqGuilvRemoveCondition	conditions){
	...
	public static void condition(CqGuilvRemoveConditionService conditionService,CqGuilvRemoveCondition condition,CqGuilv g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType,CqGuilvService guilvOldService){
		PageView<CqGuilv> pageViewGuilv = new PageView<CqGuilv>();
		List<PropertyFilter> filtersnewses = new ArrayList<PropertyFilter>();
		filtersnewses
				.add(new PropertyFilter("addition", MatchType.EQ, addition));
		...
		List<CqGuilv> gs=guilvOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
