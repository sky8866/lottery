...
public class LotteryGuilvSizeUtils {
	private	static GuilvSizeOLdService guilvOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,LotteryService lotteryService,GuilvSizeOLdService guilvService,List<GuilvSizeOld> sbr,boolean isvip,GuilvSizeOLdService oldService,GuilvRemoveCountService countService,GuilvRemoveConditionService conditionService){
		...
		PageView<GuilvSizeOld> pageViewGuilv = new PageView<GuilvSizeOld>();
		...
		filtersnewses.add(new PropertyFilter("gui", MatchType.EQ, 6));
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<GuilvSizeOld> guilvs = pageViewGuilv.getRecords();
		
		if (guilvs.isEmpty()) {
			 GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 6);
	...
	/*	ComparatorGuilvSizeOld guilv = new ComparatorGuilvSizeOld();
	...
	public static String getVaule(List<GuilvSizeOld> sbr, int type) {
		String stypeString = "";
		if (type == 0) {
			stypeString = "头";
		} else if (type == 1) {
	...
	private static void guilv(int type, int sum, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<GuilvSizeOld> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,int qi, GuilvSizeOLdService guilvService, GuilvRemoveCondition  conditions) {
	...
	private static void getFixation(int type, int sum, int query,
			List<Lottery> lotteries, List<StringBuffer> sbs, List<GuilvSizeOld> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,int qi, GuilvSizeOLdService guilvService, GuilvRemoveCondition  conditions) {
		int z = s(sum);
		int x = z + e;
	...
	private static Boolean more(int x, int query, int number) {
		...
		Integer[] nums = GuilvSizeUtils.getQueryValue(x, number);
	...
	private static void one(int x, int type, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<GuilvSizeOld> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,int qi, GuilvSizeOLdService guilvService, GuilvRemoveCondition  conditions) {
			...
			if (bsum) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					if(e==0){
						string ="判断条件为：" + qu+"";
						string1= startQu+"="+startValue+"，即"+lotteries.get(count-1).getQi()+"期为："+GuilvSizeUtils.getSumValue(startValue, number)+stypeString+",此规从"+lotteries.get(count-1).getQi()+"期开始，已经连续跑了"+count+"期。";;
					}else{
						string ="判断条件为：" + e + "+" + qu+"";
						string1= e+"+" +startQu+"="+startValue+"，即"+lotteries.get(count-1).getQi()+"期为："+GuilvSizeUtils.getSumValue(startValue, number)+stypeString+",此规从"+lotteries.get(count-1).getQi()+"期开始，已经连续跑了"+count+"期。";
								 
					}
				
				
					GuilvSizeOld vo = new GuilvSizeOld();
					...
					vo.setStartValue(GuilvSizeUtils.getSumValue(startValue, number));
					vo.setValue(GuilvSizeUtils.getSumValue(num, number));
					vo.setFixation(e);
                    ...
                    vo.setGui(6);
					 ...
					 if(StringUtils.isNotEmpty(vo.getInfo())){
						 ...
						 vo.setNum(getOldLottery(vo.getSum(),addition, buchang, even, e, qi, matchType, guilvOldService));
	...
	public static void guilvRemove(GuilvSizeOld g, GuilvRemoveCondition conditions,GuilvRemoveCountService cocuntService,GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(GuilvSizeOld g,GuilvRemoveCountService countService,int remove,GuilvRemoveCondition	conditions){
		Object[] objects=new Object[5];
		 ...
		 if(count==null){
			 ...
			 count.setInfo(g.getInfo());
	    	    ...
	    	    count.setQi(g.getQi());
	    	    count.setQueryValue("【大小规】"+g.getQueryValue());
	...
	private static void condition(GuilvRemoveCondition condition,GuilvSizeOld g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,int qi, int matchType,GuilvSizeOLdService guilvOldService){
		PageView<GuilvSizeOld> pageViewGuilv = new PageView<GuilvSizeOld>();
		List<PropertyFilter> filtersnewses = new ArrayList<PropertyFilter>();
		filtersnewses
				.add(new PropertyFilter("addition", MatchType.EQ, addition));
		...
		List<GuilvSizeOld> gs=guilvOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(GuilvSizeOld g:gs){
