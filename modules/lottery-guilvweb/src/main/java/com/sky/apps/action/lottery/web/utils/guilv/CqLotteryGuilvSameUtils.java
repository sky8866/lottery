...
public class CqLotteryGuilvSameUtils {
	private	static CqGuilvRemoveCountService guilvRemoveCountService;
	private	static CqGuilvSameOldService guilvSameOldService; 
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,CqsscLotteryService lotteryService,CqGuilvSameOldService guilvService,boolean isvip, CqGuilvRemoveCountService countService,CqGuilvRemoveConditionService conditionService){
		 even=even-2;
	  ...
	  guilvSameOldService=guilvService;
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<CqGuilvSameOld> guilvs = pageViewGuilv.getRecords();
		
		if (guilvs.isEmpty()) {
			CqGuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 12);
			...
			if (bsum) {
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					CqGuilvSameOld vo = new CqGuilvSameOld();
                    ...
                    vo.setGui(1);
                       ...
                       CqNumCountUtils.getGuilvSame(addition, buchang, fixation, number, vo, vo.getSum(), infoString);
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType){
		...
		List<CqGuilvSameOld> gs=guilvSameOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(CqGuilvSameOld g:gs){
	...
	public static void guilvRemove(CqGuilvSameOld g,CqGuilvRemoveCondition	conditions,CqGuilvRemoveCountService cocuntService,CqGuilvRemoveConditionService conditionService){
	...
	private static void removeCount(CqGuilvSameOld g,CqGuilvRemoveCountService countService,int remove,CqGuilvRemoveCondition	conditions){
	...
	public static void condition(CqGuilvRemoveConditionService conditionService,CqGuilvRemoveCondition condition,CqGuilvSameOld g ){
