...
public class CqLotteryPeerUtils {
	private	 static CqGuilvPeerOldService peerOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,CqsscLotteryService lotteryService,CqGuilvPeerOldService guilvService,List<CqGuilvPeerOld> sbr,boolean isvip,  CqGuilvRemoveCountService countService,CqGuilvRemoveConditionService conditionService){
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<CqGuilvPeerOld> guilvs = pageViewGuilv.getRecords();
		
		int query = sum(lotteries, 0, type, sbs2.get(0)); 
	 
		if (guilvs.isEmpty()) {
			 CqGuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 4);
	...
	private static void guilv(int type, int sum, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqGuilvPeerOld> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,long qi,List<StringBuffer> sbs2, CqGuilvRemoveCondition  conditions) {
		...
		if (fixation == 0) {
			...
			for (int e = 1; 10 > e; e++) {
				...
				getFixation(type, sum, query, lotteries, sbs, sbr, sumVaule,
						even, e, matchType, number, addition, buchang, qi,  sbs2,conditions);
	...
	private static void getFixation(int type, int sum, int query,
			List<CqsscLottery> lotteries, List<StringBuffer> sbs, List<CqGuilvPeerOld> sbr,
			List<String> sumVaule, int even, int fixation, int matchType, int number,int addition,int buchang,long qi,List<StringBuffer> sbs2 ,CqGuilvRemoveCondition  conditions) {
		int z = s(sum);
		int x = z + fixation;
		 	  ...
		 	  int countold = setold( fixation,type,sbs, lotteries,  number,count,sbs2); 
				...
				if (flag) {
					...
					if (type == 0) {
						...
						stypeString = "个";
						startNum=lotteries.get(count-1).getNumber5();
					}
					int num = s(temp2);
					if(num<0){
					...
					qu=qu.substring(0,qu.length()-1);
						...
						CqGuilvPeerOld vo = new CqGuilvPeerOld();
	...
	public static void guilvRemove(CqGuilvPeerOld g, CqGuilvRemoveCondition  conditions,CqGuilvRemoveCountService cocuntService,CqGuilvRemoveConditionService conditionService){
	...
	private static void removeCount(CqGuilvPeerOld g,CqGuilvRemoveCountService countService,int remove,CqGuilvRemoveCondition  conditions){
	...
	public static void condition(CqGuilvRemoveConditionService conditionService,CqGuilvRemoveCondition condition,CqGuilvPeerOld g ){
		if(g!=null&&condition!=null){
	...
	private static int getOldLottery(int sum, int addition, int buchang, int even,int fixation,long qi, int matchType ){
		...
		List<CqGuilvPeerOld> gs=peerOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(CqGuilvPeerOld g:gs){
	...
	private static int   setold(int e,int type,List<StringBuffer> sbs,List<CqsscLottery> lotteries, int number,int co,List<StringBuffer> sbs2){
		int count=0;
		int temp=co;
