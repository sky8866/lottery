...
public class LotteryKillUtils {
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,LotteryService lotteryService,CqsscGuilvService guilvService,List<CqsscGuilv> sbr,boolean isvip){
	...
	private static void getFixation(int type, int sum, int query,
			List<Lottery> lotteries, List<StringBuffer> sbs, List<CqsscGuilv> sbr,
			...
			if (bsum) {
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
                    ...
                    vo.setTypeVaule("本期的"+stypeString+"为:"+vo.getValue()+",此位置已经连续 ："+count+"期杀这些数字！");
                    vo.setGui(1);
