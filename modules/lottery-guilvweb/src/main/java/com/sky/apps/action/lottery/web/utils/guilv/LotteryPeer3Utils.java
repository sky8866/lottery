...
public class LotteryPeer3Utils {
	private	 static GuilvPeer3OldService peerOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,LotteryService lotteryService,GuilvPeer3Service guilvService,List<GuilvPeer3> sbr,boolean isvip,GuilvPeer3OldService pldService,GuilvRemoveCountService countService,GuilvRemoveConditionService conditionService){
		peerOldService=pldService;
		...
		PageView<GuilvPeer3> pageViewGuilv = new PageView<GuilvPeer3>();
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<GuilvPeer3> guilvs = pageViewGuilv.getRecords();
		
		int query = sum(lotteries, 3, type, sbs2.get(0)); 
	 
		if (guilvs.isEmpty()) {
			GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 5);
	...
	private static void guilv(int type, int sum, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<GuilvPeer3> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,int qi, GuilvPeer3Service guilvService,List<StringBuffer> sbs2,GuilvRemoveCondition  conditions) {
	...
	private static void getFixation(int type, int sum, int query,
			List<Lottery> lotteries, List<StringBuffer> sbs, List<GuilvPeer3> sbr,
			List<String> sumVaule, int even, int fixation, int matchType, int number,int addition,int buchang,int qi, GuilvPeer3Service guilvService,List<StringBuffer> sbs2,GuilvRemoveCondition  conditions) {
		int z = s(sum);
		int x = z + fixation;
	...
	public static void guilvRemove(GuilvPeer3Old g,GuilvRemoveCondition conditions,GuilvRemoveCountService cocuntService,GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(GuilvPeer3Old g,GuilvRemoveCountService countService,int remove,GuilvRemoveCondition	conditions){
	...
	private static void condition(GuilvRemoveCondition condition,GuilvPeer3Old g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,int qi, int matchType ){
		...
		List<GuilvPeer3Old> gs=peerOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(GuilvPeer3Old g:gs){
	...
	public static String getQueryValue(List<Lottery> lotteries, int j, int type) {
		String lString = "";
		try{
