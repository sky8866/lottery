...
public class TJsscGuilvKillUtils {
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,TjsscLotteryService lotteryService, TJsscGuilvKillService guilvService,List<TJsscGuilvKill> sbr,boolean isvip,List<TjsscLottery> lotteries){
		...
		PageView<TJsscGuilvKill> pageViewGuilv = new PageView<TJsscGuilvKill>();
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<TJsscGuilvKill> guilvs = pageViewGuilv.getRecords();
	...
	private static void guilv(int type, int sum, int query, List<TjsscLottery> lotteries,
			List<StringBuffer> sbs, List<TJsscGuilvKill> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,long qi, TJsscGuilvKillService guilvService) {
	...
	private static void getFixation(int type, int sum, int query,
			List<TjsscLottery> lotteries, List<StringBuffer> sbs, List<TJsscGuilvKill> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,long qi, TJsscGuilvKillService guilvService) {
		int z = s(sum);
		int x = z + e;
	 ...
	 * 单结果
	 */
	private static void one(int x, int type, int query, List<TjsscLottery> lotteries,
			List<StringBuffer> sbs, List<TJsscGuilvKill> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,long qi, TJsscGuilvKillService guilvService) {
			...
			if (bsum) {
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					TJsscGuilvKill vo = new TJsscGuilvKill();
	...
	private static int   setold(int e,int type,List<StringBuffer> sbs,List<TjsscLottery> lotteries, int number,int co){
		int count=0;
		int temp=co;
	...
	private static String getQueryValue(List<TjsscLottery> lotteries, int j, int type) {
