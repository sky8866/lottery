...
public class Pl5GuilvKillUtils {
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,Pl5LotteryService lotteryService, Pl5GuilvKillService guilvService,List<Pl5GuilvKill> sbr,boolean isvip,List<Pl5Lottery> lotteries){
		...
		PageView<Pl5GuilvKill> pageViewGuilv = new PageView<Pl5GuilvKill>();
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<Pl5GuilvKill> guilvs = pageViewGuilv.getRecords();
	...
	private static void guilv(int type, int sum, int query, List<Pl5Lottery> lotteries,
			List<StringBuffer> sbs, List<Pl5GuilvKill> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,long qi, Pl5GuilvKillService guilvService) {
	...
	private static void getFixation(int type, int sum, int query,
			List<Pl5Lottery> lotteries, List<StringBuffer> sbs, List<Pl5GuilvKill> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,long qi, Pl5GuilvKillService guilvService) {
		int z = s(sum);
		int x = z + e;
	...
	private static void one(int x, int type, int query, List<Pl5Lottery> lotteries,
			List<StringBuffer> sbs, List<Pl5GuilvKill> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,long qi, Pl5GuilvKillService guilvService) {
			...
			if (bsum) {
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					Pl5GuilvKill vo = new Pl5GuilvKill();
