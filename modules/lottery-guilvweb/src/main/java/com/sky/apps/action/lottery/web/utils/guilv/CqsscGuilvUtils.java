...
public class CqsscGuilvUtils {
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,CqsscLotteryService lotteryService,CqsscGuilvService guilvService,List<CqsscGuilv> sbr,boolean isvip,List<CqsscLottery> lotteries){
	...
	private static void guilv(int type, int sum, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqsscGuilv> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,int qi, CqsscGuilvService guilvService) {
	...
	private static void getFixation(int type, int sum, int query,
			List<CqsscLottery> lotteries, List<StringBuffer> sbs, List<CqsscGuilv> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,int qi, CqsscGuilvService guilvService) {
		int z = s(sum);
		int x = z + e;

		one(x, type, query, lotteries, sbs, sbr, sumVaule, even, e, matchType,
				number, addition, buchang, qi,  guilvService);
		/*two(x, type, query, lotteries, sbs, sbr, sumVaule, even, e, matchType,
				number, addition, buchang, qi,  guilvService);*/
	}
	 ...
	 * 多结果
	 */
	private static Boolean moreTwo(int x, int query,int query2, int number) {
	...
	private static void one(int x, int type, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqsscGuilv> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,int qi, CqsscGuilvService guilvService) {
	 ...
	 * 单结果
	 */
	private static void two(int x, int type, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqsscGuilv> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,int qi, CqsscGuilvService guilvService) {
          if(type==0||type==3){
			...
			int c=0;
			  ...
			  for (int j = 1; 200 > j; j++) {
				...
				if (type == 0) {
					qq = lotteries.get(j).getNumber1();
					qq1 = lotteries.get(j).getNumber2();
				} else if (type == 3) {
					qq = lotteries.get(j).getNumber4();
					qq1 = lotteries.get(j).getNumber5();
				} 
				if (moreTwo(s(temp2), qq,qq1, number)) {
					startValue=s(temp2);
					startQu=startQuTemp;
						countTemp++;	
			 ...
			 if(countMin>=14&&errorNum<3&&errorCou<12){ 
