...
public class LotteryGuilvScopeUtils {
	private	static GuilvRemoveCountService guilvRemoveCountService;
	private	static GuilvScopeOLdService oScopeOLdService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,LotteryService lotteryService,GuilvScopeService guilvService,List<GuilvScope> sbr,boolean isvip,GuilvScopeOLdService scopeOLdService,GuilvRemoveCountService countService,GuilvRemoveConditionService conditionService){
		guilvRemoveCountService=countService;
		  oScopeOLdService=scopeOLdService;
		...
		PageView<GuilvScope> pageViewGuilv = new PageView<GuilvScope>();
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<GuilvScope> guilvs = pageViewGuilv.getRecords();
		
		if (guilvs.isEmpty()) {
			GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 2);
	...
	private static void guilv(int type, int sum, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<GuilvScope> sbr, List<String> sumVaule,
			int even, int fixation, int matchType, int number,int addition,int buchang,int qi, GuilvScopeService guilvService,GuilvRemoveCondition  conditions) {
	...
	private static void getFixation(int type, int sum, int query,
			List<Lottery> lotteries, List<StringBuffer> sbs, List<GuilvScope> sbr,
			List<String> sumVaule, int even, int e, int matchType, int number,int addition,int buchang,int qi, GuilvScopeService guilvService,GuilvRemoveCondition  conditions) {
		int z = s(sum);
		int x = z + e;
	...
	private static void one(int x, int type, int query, List<Lottery> lotteries,
			List<StringBuffer> sbs, List<GuilvScope> sbr, List<String> sumVaule,
			int even, int e, int matchType, int number,int addition,int buchang,int qi, GuilvScopeService guilvService,GuilvRemoveCondition  conditions) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
                    ...
                    vo.setGui(1);
                       ...
                       NumCountUtils.getGuilvScope(addition, buchang, fixation, number, vo, vo.getSum(), infoString);
     
					 guilvService.saveGuilv(vo); 
					 if(StringUtils.isNotEmpty(vo.getInfo())){
						 GuilvScopeOld old=new GuilvScopeOld(vo);
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,int qi, int matchType){
		...
		List<GuilvScopeOld> gs=oScopeOLdService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(GuilvScopeOld g:gs){
	...
	public static void guilvRemove(GuilvScopeOld g,GuilvRemoveCondition conditions,GuilvRemoveCountService cocuntService,GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(GuilvScopeOld g,GuilvRemoveCountService countService,int remove,GuilvRemoveCondition	conditions){
	...
	private static void condition(GuilvRemoveCondition condition,GuilvScopeOld g ){
