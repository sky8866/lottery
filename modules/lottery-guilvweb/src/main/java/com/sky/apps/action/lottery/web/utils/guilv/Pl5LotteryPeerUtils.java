...
public class Pl5LotteryPeerUtils {
	private	 static Pl5GuilvPeerOldService peerOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,Pl5LotteryService lotteryService,Pl5GuilvPeerOldService guilvService,List<Pl5GuilvPeerOld> sbr,boolean isvip,  Pl5GuilvRemoveCountService countService,Pl5GuilvRemoveConditionService conditionService){
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<Pl5GuilvPeerOld> guilvs = pageViewGuilv.getRecords();
		
		int query = sum(lotteries, 0, type, sbs2.get(0)); 
	 
		if (guilvs.isEmpty()) {
			Pl5GuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 4);
	...
	private static void guilv(int type, int sum, int query, List<Pl5Lottery> lotteries,
			List<StringBuffer> sbs, List<Pl5GuilvPeerOld> sbr, List<String> sumVaule,
	...
	private static void getFixation(int type, int sum, int query,
			List<Pl5Lottery> lotteries, List<StringBuffer> sbs, List<Pl5GuilvPeerOld> sbr,
		 	  ...
		 	  int countold = setold( fixation,type,sbs, lotteries,  number,count,sbs2); 
				...
				if (flag) {
					...
					qu=qu.substring(0,qu.length()-1);
						...
						Pl5GuilvPeerOld vo = new Pl5GuilvPeerOld();
	...
	public static void guilvRemove(Pl5GuilvPeerOld g,Pl5GuilvRemoveCondition conditions,Pl5GuilvRemoveCountService cocuntService,Pl5GuilvRemoveConditionService conditionService){
	...
	private static void removeCount(Pl5GuilvPeerOld g,Pl5GuilvRemoveCountService countService,int remove,Pl5GuilvRemoveCondition	conditions){
	...
	public  static void condition(Pl5GuilvRemoveCondition condition,Pl5GuilvPeerOld g,Pl5GuilvRemoveConditionService conditionService ){
	...
	private static int getOldLottery(int sum, int addition, int buchang, int even,int fixation,long qi, int matchType ){
		...
		List<Pl5GuilvPeerOld> gs=peerOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(Pl5GuilvPeerOld g:gs){
