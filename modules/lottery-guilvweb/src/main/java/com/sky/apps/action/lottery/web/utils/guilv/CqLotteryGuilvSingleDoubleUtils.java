...
public class CqLotteryGuilvSingleDoubleUtils {
	private	static CqGuilvSingleDoubleOldService guilvOldService;
	...
	public  static void setQuery(int addition,
			 ...
			 int fixation,
			  int matchType,
			 int number,CqsscLotteryService lotteryService,CqGuilvSingleDoubleOldService guilvService,List<CqGuilvSingleDoubleOld> sbr,boolean isvip,CqGuilvRemoveCountService countService,CqGuilvRemoveConditionService conditionService){
		 even=even-2;
		...
		pageViewGuilv.setQueryResult(guilvService.findList(pageViewGuilv,
				filtersnewses));
		List<CqGuilvSingleDoubleOld> guilvs = pageViewGuilv.getRecords();
		
		if (guilvs.isEmpty()) {
			  CqGuilvRemoveCondition  conditions=conditionService.findGuilv(fixation, buchang, addition, number, 3);
	...
	public static String getVaule(List<CqGuilvSingleDoubleOld> sbr, int type) {
		String stypeString = "";
		if (type == 0) {
			stypeString = "万";
		} else if (type == 1) {
		...
		for (CqGuilvSingleDoubleOld vo : sbr) {
	...
	private static void guilv(int type, int sum, int query, List<CqsscLottery> lotteries,
			List<StringBuffer> sbs, List<CqGuilvSingleDoubleOld> sbr, List<String> sumVaule,
	...
	private static void getFixation(int type, int sum, int query,
			List<CqsscLottery> lotteries, List<StringBuffer> sbs, List<CqGuilvSingleDoubleOld> sbr,
			...
			if (bsum) {
		 	  ...
		 	  int countold = setold( e,type,sbs, lotteries,  number,count); 
				...
				if (flag) {
					...
					CqGuilvSingleDoubleOld vo = new CqGuilvSingleDoubleOld();
...
public static void guilvRemove(CqGuilvSingleDoubleOld g,CqGuilvRemoveCondition	conditions,CqGuilvRemoveCountService cocuntService,CqGuilvRemoveConditionService conditionService){
	...
	private static void removeCount(CqGuilvSingleDoubleOld g,CqGuilvRemoveCountService countService,int remove,CqGuilvRemoveCondition	conditions){
	...
	public static void condition(CqGuilvRemoveConditionService conditionService,CqGuilvRemoveCondition condition,CqGuilvSingleDoubleOld g ){
	...
	private static int getOldLottery(int sum,int addition, int buchang, int even,int fixation,long qi, int matchType){
		...
		List<CqGuilvSingleDoubleOld> gs=guilvOldService.findList(pageViewGuilv, filtersnewses).getResultlist();
		 
		int num=0;
		for(CqGuilvSingleDoubleOld g:gs){
